...
class StableSortNode final : public DOpNode<ValueType>
{
...
public:
    ...
    StableSortNode(const ParentDIA& parent,
             const CompareFunction& compare_function,
             const SortAlgorithm& sort_algorithm = SortAlgorithm())
        : Super(parent.ctx(), "StableSort", { parent.id() }, { parent.node() }),
...
template<typename ValueType, typename Stack, typename CompareFunction, typename SortAlgorithm>
inline auto StableSort(
    ...
    static_assert(
        std::is_convertible<
            ValueType,
            typename common::FunctionTraits<CompareFunction>::template arg<0> >::value,
    ...
    auto node = tlx::make_counting<StableSortNode<ValueType, CompareFunction, SortAlgorithm>>(
        dia, compare_function, sort_algorithm);
